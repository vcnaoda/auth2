...
function initFirestore() {
  ...
  try {
    ...
    console.log('✅ Firestore habilitado:', FIRESTORE_DB_DOC);
    return firestore;
  } catch (e) {
...
async function loadDB() {
  try {
    ...
    if (fsDb) {
      ...
      const snap = await fsDb.collection(col).doc(doc).get();
...
async function saveDB(db) {
  try {
    ...
    if (fsDb) {
      const [col, doc] = safe(FIRESTORE_DB_DOC).split('/');
      await fsDb.collection(col).doc(doc).set(
        {
...
function validateSecretId(db, secretId, appId) {
  ...
  // OBS: o caller salva no final (Firestore/local)
...
const server = http.createServer((req, res) => {
  ...
  if (parsedUrl.pathname === '/api/validate' && req.method === 'GET') {
    handleValidate(req, res).catch(e => {
  ...
  if (parsedUrl.pathname === '/api/pause-key' && req.method === 'POST') {
    ...
    req.on('end', () => {
      (async () => {
        ...
        console.error('[pause-key] erro:', e);
        res.writeHead(500, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
        res.end(JSON.stringify({ success: false, message: 'Erro interno', code: 'INTERNAL_ERROR' }));
      });
    });
  ...
  if (parsedUrl.pathname === '/api/reset-key' && req.method === 'POST') {
    ...
    req.on('end', () => {
      (async () => {
        ...
        console.error('[reset-key] erro:', e);
        res.writeHead(500, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
        res.end(JSON.stringify({ success: false, message: 'Erro interno', code: 'INTERNAL_ERROR' }));
      });
    });
  ...
  if (parsedUrl.pathname === '/api/reset-hwid' && req.method === 'POST') {
    ...
    req.on('end', () => {
      (async () => {
        ...
        console.error('[reset-hwid] erro:', e);
        res.writeHead(500, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
        res.end(JSON.stringify({ success: false, message: 'Erro interno', code: 'INTERNAL_ERROR' }));
      });
    });
  ...
  if (parsedUrl.pathname === '/api/delete-key' && req.method === 'POST') {
    ...
    req.on('end', () => {
      (async () => {
        ...
        console.error('[delete-key] erro:', e);
        res.writeHead(500, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
        res.end(JSON.stringify({ success: false, message: 'Erro interno', code: 'INTERNAL_ERROR' }));
      });
    });
  ...
  if (parsedUrl.pathname === '/api/key-status' && req.method === 'GET') {
    ...
    return;
      ...
      console.error('[key-status] erro:', e);
      res.writeHead(500, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
      res.end(JSON.stringify({ success: false, message: 'Erro interno', code: 'INTERNAL_ERROR' }));
    });
    return;
  ...
  if (parsedUrl.pathname === '/api/create-key' && req.method === 'POST') {
    ...
    req.on('end', () => {
      (async () => {
        ...
        console.error('[create-key] erro:', e);
        res.writeHead(500, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
        res.end(JSON.stringify({ success: false, message: 'Erro interno', code: 'INTERNAL_ERROR' }));
      });
    });
  ...
  if (parsedUrl.pathname === '/db.json' && req.method === 'GET') {
    (async () => {
      ...
      console.error('[/db.json GET] erro:', e);
      res.writeHead(500, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' });
      res.end(JSON.stringify({ success: false, message: 'Erro interno', code: 'INTERNAL_ERROR' }));
    });
    return;
